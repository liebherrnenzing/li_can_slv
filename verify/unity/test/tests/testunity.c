...
void testBinrayFileCompare(void)
{
    ...
    TEST_IGNORE();
#else
    TEST_ASSERT_BINARY_FILE("support/example.bin", "support/example.bin");
#endif
}
...
#else
    EXPECT_ABORT_BEGIN
    TEST_ASSERT_BINARY_FILE(NULL, "support/example.bin");
    VERIFY_FAILS_END
#endif
}
...
#else
    EXPECT_ABORT_BEGIN
    TEST_ASSERT_BINARY_FILE("support/example.bin", NULL);
    VERIFY_FAILS_END
#endif
}
...
#else
    EXPECT_ABORT_BEGIN
    TEST_ASSERT_BINARY_FILE("support/example.bin", "support/example2.bin");
    VERIFY_FAILS_END
#endif
}
